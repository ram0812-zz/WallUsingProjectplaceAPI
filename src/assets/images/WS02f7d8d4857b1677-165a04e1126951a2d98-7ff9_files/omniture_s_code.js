...
if (!window.s_accountName && !window.s_account) { 
	...
	else if (s_Host.indexOf("createpdf.acrobat.com") != -1) {
		var s_account="mxmacromedia,mxcreatepdf";
	}
	else if (s_Host.indexOf("formscentral.acrobat.com") != -1) {
		var s_account="mxmacromedia,mxformscentral";
	}
	else if (s_Host.indexOf("sendnow.acrobat.com") != -1) {
	  var s_account="mxmacromedia,mxsendnow";
	}
	else if (s_Host.indexOf("acrobat.com") != -1) {
	  var s_account="mxmacromedia,mxacrobatdotcom";
	}
...
} else if (window.s_accountName!='' && window.s_account==null){
                var s_account=window.s_accountName;
}
...
/* Eloqua Integration */	
	...
	if (s_matchElq < 1) {
		...
		else if ((s_URL.indexOf("/cfusion/event/index.cfm?event=list") != -1) && (s_URL.indexOf("loc=en_us") != -1)) {
			s_matchElq = 1;	
		}
		else if (s_URL.indexOf("/cfusion/acrobatconnectpro") != -1) {
			s_matchElq = 1;
		}
		
		else if (s_URL.indexOf("/cfusion/showcase/") != -1) {
			s_matchElq = 1;
		}	
		
		else if (s_URL.indexOf("/cfusion/mmform/") != -1) {
			s_matchElq = 1;
		}
					
		else if (s_URL.indexOf("/cfusion/tdrc/ ") != -1) {
			s_matchElq = 1;
		}			
	...
	/* TnT Load JS File, currently exclude Polar Bear */
...
/*
* Partner Plugin: Demandbase TL v1.3 (BEGIN)
*/
...
function s_doPlugins(s) {
  ...
  if ((window.s_account=="mxmacromedia") && typeof(s_pageName)!=="undefined" && window.s_URL.indexOf("/products/")!=-1) {
      if (s_pageName.indexOf('ad_')==0) {
  	  	s_pageName=s_pageName.replace(/\?(.*)/, ""); // strip query string
	  	var s_splitpageName=s_pageName.split('_'); 
	  	var s_productFirst=s_splitpageName[2].slice(0,1); // Capture first letter of productname
	  	if (s_splitpageName[1]=='en') s_splitpageName[1]='us'; // change from en to us
	  	var s_productURL=window.s_URL.substring(s_URL.indexOf("/products/")+9); // Get product from URL
	  	s_splitURL=s_productURL.split('/');
		if(s_splitURL.length<4) {
			s_URLproduct=s_splitURL[1];
			if(s_URLproduct=="creativesuite") {
				if (s_splitURL[2]=="web.html" || s_splitURL[2]=="design.html" || s_splitURL[2]=="designpremium.html") {
					s_URLproduct=s_splitURL[2];
					s_splitpageName[2]=s_URLproduct.replace(/\.(.*)/,""); // strip file extension;
				}
			} 			
		}
        else if (s_splitURL.length>=4) s_URLproduct=s_splitURL[2];
	  	var s_fileName=s_splitpageName[3].split('.');
	  	s_URLproduct=s_URLproduct.replace(/\.(.*)/,""); // strip file extension

	  	if (s_splitpageName[2] == s_fileName[0] || s_productFirst!=s_productFirst.toUpperCase()) {
	  		if(s_URLproduct=='design') s_URLproduct='DesignPremium';
	  		else if(s_URLproduct=='designstandard') s_URLproduct='DesignStandard';
	  		else if (s_URLproduct=='web') s_URLproduct='WebPremium';
	  		else if (s_URLproduct=='acrobatpro') s_URLproduct='AcrobatPro';
	  		else if (s_URLproduct=='acrobatstandard') s_URLproduct='AcrobatStandard';	
	  		else if (s_URLproduct=='acrobat') s_URLproduct='AcrobatFamily';
	  		else if (s_URLproduct=='creativesuite') s_URLproduct='CreativeSuiteFamily';
	  		else if (s_URLproduct=='aftereffects') s_URLproduct='AfterEffects';
	  		else if (s_URLproduct=='premiere') s_URLproduct='PremierePro';
	      	s_URLproduct=s_URLproduct.slice(0,1).toUpperCase()+s_URLproduct.slice(1);	
	      	s_splitpageName[2]=s_URLproduct;
	  	}
		/* Rewrite Variables */
		s_channel="ad_"+s_splitpageName[2];
		s.channel=s_channel;	
		if (s_URLproduct!="AcrobatFamily" && s_URLproduct!="CreativeSuiteFamily" && s_pageName.indexOf("InlineConfigurator")==-1 && s_pageName.indexOf("HoverCart")==-1 && s_pageName.indexOf("Hovercart")==-1) { 
			s_products=";"+s_splitpageName[2]+";;;;evar26=site|evar27="+s_channel;
			s.products=s_products;
		}
		else if (s_pageName.indexOf("Hovercart")!=-1) {
			s_events="event83";
			s.events="event83";
		}
		else if (s_pageName.indexOf("HoverCart")==-1 && s_pageName.indexOf("Hovercart")==-1) {
			s_products="";
			s.products="";
		}
		s_pageName=s_splitpageName[0];
		for (i=1;i<s_splitpageName.length;i++) {
	  		s_pageName=s_pageName+"_"+s_splitpageName[i];	
		}
		s.pageName=s_pageName;
		s_prop32="ad_"+s_splitpageName[1];
		s.prop32=s_prop32;
		if(s_prop52.indexOf("ad_")!= -1) {
			s_prop52="ad_"+s_splitpageName[1]+"_"+s_splitpageName[2];
			s.prop52=s_prop52;
		}
    }
}  else if (typeof(s_pageName)!=="undefined" && window.s_URL.indexOf("/cart.html")!=-1) {
	if (s_pageName.indexOf('ad_') == 0) {
  	  s_pageName=s_pageName.replace(/\?(.*)/, ""); // strip query string
	  var s_splitpageName=s_pageName.split('_'); 
	  var s_productFirst=s_splitpageName[2].slice(0,1); // Capture first letter of productname
...
/* SocialPlatforms and SocialTerms */
  ...
  s.eVar50=!s.getQueryParam('skwcid')&&s.getQueryParam('sdid')?'Other Channel':'';   
  /* Persist SEMvar for 45 days and populate prop51 */
  s.prop51=s.getAndPersistValue(s.SEMvar,'s_cp_persist',45);
 ...
 /* TnT Execute Plugin */
  if (s.c_r("adobe_optout")!=1 && s_URL.indexOf("/misc/optout")==-1) {
   if ((s_matchTnT < 0) && (s_URL.indexOf("acrobat.com") == -1) && (s_URL.indexOf("kuler.adobe") == -1) && (s_URL.indexOf("/flash/about") == -1) && (s_URL.indexOf("/readstep") == -1) && (s_URL.indexOf("/flashplayer") == -1) && (s_URL.indexOf("photoshop.com") == -1) && (s_URL.indexOf("/reader") == -1) && (s_URL.indexOf("createpdf") == -1) && (s_URL.indexOf("blogs.adobe") == -1) && (s_URL.indexOf("/special/ratingsreviews") == -1)) {
	// if(typeof(window['mboxVersion']) != "undefined") mboxLoadSCPlugin(s);
	 mboxLoadSCPlugin(s);
...
+"D=s.split(K[i],'>');if(g.indexOf(D[0])!=-1){if(a){s[a]=D[1];console" 
+".log(D[1]);s[a]=s.getValOnce(s[a],'s_sPlat');}}}"); 

s.socPlatList="facebook.com>Facebook|twitter.com>Twitter|t.co/>Twitter|youtube.com>Youtube";
...
+"a]=M;s[a]=s.getValOnce(s[a],'s_sTerms');");
...
if ((s_account=="mxmacromedia") && (typeof(Adobe)!==undefined) && (s_URL.indexOf("/products/")==-1) && (s_URL.indexOf("/solutions.html")!=-1||s_URL.indexOf("/solutions/")!=-1)) {
 s.server = "Day WCMS";
 var s_code=s.t();
